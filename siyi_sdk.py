...
class SIYISDK:
    ...
    def set_zoom(self, level):
        ...
        if self.cam_model == 'a8mini':
            print('A8MINI does not support absolute zoom. Abort.')
            return True
        if level > 30:
            level = 30
        elif level < 1:
    ...
    def PID_contol(self):
        ...
        while True:
            ...
            if self._motionMode_msg.mode != MotionModeMsg.FOLLOW:
                print("PID on pause. Current Mode is", self._motionMode_msg.mode)
                sleep(1)
                continue
            else:
                self.is_pause_pid = False
